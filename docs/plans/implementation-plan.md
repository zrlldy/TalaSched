...
- [ ] Create policies for organizations, memberships, invitations, academic records, resources, schedules, versions, approvals, templates, subscriptions, and audit records.
    - **Progress:** Added auto-discovered tenant policies for current organization, membership, invitation, academic, resource, catalog, timetable, version, schedule, custom role, and approval signatory-profile models. Signatory reads and mutations require both normalized scheduling management permission and the `approval_workflows` entitlement; the signatory list, signed download, and mutation requests enforce that policy while direct service calls retain the same domain guard. Workflow tables, templates, subscriptions, and audit records remain pending until their schema-only areas have models and command surfaces. Focused policy/signatory coverage and the full Pest suite pass (323 tests, 1,955 assertions).
...
- [ ] Integrate audit logging into organization, membership, academic, resource, scheduling, version, approval, subscription, template, export, and security-sensitive actions.
    - **Progress:** Version lifecycle, approval submission/decision, organization lifecycle, initial subscription provisioning, membership changes, invitation lifecycle, custom-role administration, all currently implemented academic/resource/catalog command services, account registration, profile updates, self-deletion, password resets, and authenticated password changes now record transactionally. Profile records contain only boolean change summaries; password records deliberately contain no credential or token payload. Further subscription changes, template, export, and remaining security-sensitive integrations remain.
- [ ] Capture actor, organization, target, request metadata, before/after summaries, correlation ID, and impersonation context without storing secrets.
    - **Progress:** `AuditLogger` records client IP/user agent, reuses the active request correlation ID, prefers public subject identifiers, and records the active `impersonator_user_id` context when a delegated-access flow establishes one. Organization-less account events use a scoped PostgreSQL insert policy and preserve historical actor identifiers after account deletion. It does not log request bodies, headers, credentials, or tokens. Systematic action integration remains incomplete.
...
- [ ] Enforce MIME, extension, ZIP-entry, decompression-size, macro, and formula/external-link policies for uploaded workbooks and signature assets.
    - **Progress:** Signature uploads now verify decoded image type, extension, MIME type, size, and a 4096-pixel maximum for each dimension at both HTTP and direct service boundaries. Workbook ZIP-entry, decompression-size, macro, and formula/external-link policies remain blocked on the approved workbook dependency and command surface.
...
- [ ] Add rate limits for invitations, schedule validation, exports, uploads, and future generation jobs.
    - **Progress:** Organization-and-actor-scoped limits protect invitation creation/cancellation (10/minute), invitation responses (10/minute), schedule validation (30/minute), and signatory uploads (10/minute). Focused coverage verifies invitation throttling and the JSON scheduling `429` envelope; exports and generation jobs remain pending until those command routes exist.
...
- [ ] Add a permission-aware application navigation organized around Setup, Resources, Scheduling, Approvals, Templates, Reports, Billing, and Audit.
    - **Progress:** Current organization members receive only implemented, authorized route groups: Workspace, Setup, Resources, Scheduling when a concrete timetable context exists, Approvals, and Billing for organization administrators. Templates, Reports, and Audit navigation awaits their underlying authorized page contracts.
...
- [ ] Implement scheduling, version comparison, approvals, template mapping, export, billing, and audit pages described in earlier phases.
    - **Progress:** Scheduling, version-ledger and comparison, approval inbox/workflow/signatory, and billing screens are implemented with their corresponding authorization contracts. Template mapping, export, and audit pages remain pending their Phase 10 command and permission contracts.
...
- [ ] Add browser smoke tests for the critical owner, scheduler, approver, teacher/viewer, and multi-organization journeys.
    - **Progress:** Browser coverage is blocked pending explicit approval to add Pest's browser plugin and its Playwright runtime; the repository currently has no browser-test suite or installed browser-test package.
