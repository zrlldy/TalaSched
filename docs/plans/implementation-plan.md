...
- [ ] Add authorization, replay/idempotency, concurrent-action, historical-snapshot, and workflow-version tests.
    - **Progress:** Authorization, idempotency, invalid-version rollback, sequential advancement, resubmission, workflow activation, retirement, signatory snapshot, and controller contract coverage pass locally. Added a PostgreSQL-gated concurrent decision lock-race regression; PostgreSQL execution and broader historical workflow-version coverage remain pending.
