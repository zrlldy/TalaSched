...
const selectorLabel = (step: WorkflowStep): string =>
    step.selector_type === 'permission'
        ? (step.required_permission?.replaceAll(':', ' · ') ?? 'Permission')
        : step.role_codes.join(', ');
...
<template>
    ...
    <div class="space-y-8">
        <WorkspacePageHeader
            section="Approvals"
            title="Workflow designer"
            description="Create immutable workflow versions, inspect their selectors, and activate the version future submissions will use."
        >
            <template #actions
                ><Button variant="outline" as-child
                    ><a href="#new-workflow">New workflow</a></Button
                ></template
            >
        </WorkspacePageHeader>
        ...
        <section id="new-workflow" class="rounded-lg border bg-card p-5 sm:p-7">
            <div class="flex items-start gap-3">
                ...
                <div>
                    ...
                    <p class="mt-1 text-sm leading-6 text-muted-foreground">
                        Each save creates a new immutable version. Activate it
                        only after all selectors and steps are ready.
                    </p>
                </div>
            </div>
            <Form
            ...
            >
                ...
                <div class="flex flex-wrap justify-between gap-3">
                    <Button type="button" variant="outline" @click="addStep"
                        ><Plus /> Add step</Button
                    ><Button type="submit" :disabled="processing"
                        ><Check /> Save draft version</Button
                    >
                </div>
            </Form>
        </section>
        ...
        <WorkspaceState
            ...
            description="Create your first workflow version above to make timetable approvals available."
        />
                ...
                <div class="flex items-start justify-between gap-3">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-muted-foreground">
                            {{ workflow.versions.length }} immutable version{{
                    ...
                    <Badge
                        ...
                        :class="
                            workflow.is_active
                                ? 'border-schedule/30 bg-schedule/10 text-schedule'
                                : 'border-border bg-muted text-muted-foreground'
                        "
                        >{{ statusLabel(workflow.is_active) }}</Badge
                    >
                </div>
                <div class="mt-5 space-y-3">
                    ...
                    >
                        ...
                        <ol class="mt-4 space-y-2">
                            ...
                            >
                                <span
                                    class="grid size-5 shrink-0 place-items-center rounded-full bg-muted text-xs"
                                    >{{ step.sequence }}</span
                                >
                                <div>
